...
| Web | React / Next.js | 站点、Profile、规则、Contract、任务、diff、质量、安全、预算 |
...
| 监控 | Prometheus + Grafana + OpenTelemetry | 系统 SLO + 业务质量 + Profile/Contract drift |
...
约束：`UNIQUE(site_id, url_hash)`；一个 URL 可以由 Feed、Sitemap、Platform Archive、Search Index、Common Crawl 等多个来源共同证明。
...
采用 weighted fair queue：日常增量高于历史 backfill，单个大站不能垄断全部 Worker。PG/S3 延迟、Redis pending、Extract backlog、Browser RSS、CPU/RSS 或 object-store throughput 异常时，Scheduler 自动降低上游速度。
...
全量 backfill、单站增量、reconciliation、source probing、单 URL 重抓、re-extract without refetch、asset retry、export、enrichment、cancel/pause/resume、job/item 进度和重试。
...
显示 domain limiter wait、HTTP/Browser/Extract slots、Browser RSS、Provider 请求量/字节/费用、scroll count、budget exhaustion、SSRF deny、robots deny、access restricted 和异常 active content。
...
### Phase 2：历史覆盖、规则编译与质量强化

Common Crawl/Wayback、可选 Search Index Source Discovery、soft-404、普通归档/Deep Crawl、golden/shadow Workbench、Extraction Contract / Schema Synthesis、Profile/Contract release regression、Extractor capability registry、contract-test pipeline、metadata conflict review、drift 和完整 Control Matrix。
...
```text
production selector/quality failures
