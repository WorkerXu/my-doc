...
10. 抓取原始字节、抽取结果、规则、抽取器、质量门禁和 Serializer 全部可追溯，使任意文章版本都能解释“为什么变”。
...
6. **不可变抓取快照优先保留**；HTTP 原始 response bytes 是回放的权威输入，规则、抽取器、质量门禁和 Serializer 变化优先离线 replay。
7. **规则、Canonicalizer、Fetcher、Capture Profile、Extractor、Extractor Capability、Quality Gate、Serializer、Security Policy 全部版本化**。
...
19. **Extractor 能力必须显式注册**；不能假设不同 Adapter 都返回相同 metadata、HTML、图片、链接或质量分语义。
20. **`asyncio.to_thread()` 只代表把同步调用移出 event loop，不代表已获得多核 CPU 并行**；Extract 默认按独立进程/容器隔离和压测后的有界并发运行。
...
flowchart LR
    ...
    REDIS --> EX[Extract Workers / CPU Process Pool]
...
- **抽取面**：多 Extractor Adapter、Extractor Capability Registry、Quality Gate、Article IR、Markdown Serializer。
- **协调面**：Redis Streams、DB lease、outbox、分布式限流、backpressure。
- **治理面**：Egress、SSRF、robots、输入预算、预览隔离、日志脱敏、供应链、第三方组件 contract test。
...
| 通用正文抽取 | `RsTrafilaturaDirectAdapter` + Crawl4AI Generic Adapter | rs-trafilatura 优先直连 Python binding；候选结果进入平台 Quality Gate |
| Crawl4AI rs-trafilatura 集成 | `RsTrafilaturaStrategy` 兼容路径 | 适合简单/内联验证，不作为平台唯一抽取 contract |
| Extract Runtime | 独立进程/容器，必要时 process pool | CPU/内存与 Fetch 隔离；线程并行必须经基准确认 |
...
| 安全扫描 | OSV/pip-audit + Bandit/Semgrep + SBOM | 依赖、wheel/native extension 和供应链 |
...
声明式规则支持 include/exclude path、CSS/XPath、归档分页、作者/日期/tag selector、canonical/identity、Browser wait 条件、approved hosts、per-site rate/budget、page-type override 和 extractor priority。
...
- HTTP `body_sha256` 基于**原始响应 bytes**计算；原始 bytes 是后续离线抽取和编码重判的权威输入。
- 不只保存“平台第一次 decode 后的字符串”。旧站 charset 错误时，未来可以重新用新的解码器/抽取器回放。
- Browser 必须区分服务器原始响应和渲染后 DOM；Browser rendered DOM 是 Unicode 文档输入，不冒充 HTTP raw bytes。

### 5.5 Extractor Release / Capability Registry
...
规则引擎只使用 release 声明过的能力，禁止把某一 Adapter 的返回字段隐式推广给所有 Extractor。
...
```text
UNIQUE(snapshot_id, extractor_release_id, config_hash, input_kind)
```

第三方 `raw_quality_score` 不直接决定发布，且必须连同 `raw_quality_kind` 一起解释；`platform_quality_score` 才是经过版本化规则得到的平台质量结果。
...
6. 对 golden snapshots 运行 shadow extraction，对比 `RsTrafilaturaDirectAdapter`、Crawl4AI generic 和站点 selector（若有）。
7. 自动生成 include/exclude、identity/canonical、metadata、extractor priority、quality threshold、page-type override 候选规则。
...
4. streaming body，限制 bytes 和 wall time，并将原始 bytes 流式写入对象存储/临时文件。
5. 校验 content-type，记录 header charset、sniff/decoder observation，但不以第一次 decode 结果替代 raw bytes。
...
```text
extractor/version/adapter_version/config_hash
input kind / input sha256
page type / classification confidence
raw quality kind / raw quality score
正文 text / normalized html / blocks
metadata candidates
links / assets
warnings
cpu_ms / wall_ms / peak_rss / bytes
```

推荐优先级：

1. `SiteSelectorExtractor`：站点配置了稳定 selector 时优先。
2. `RsTrafilaturaDirectAdapter`：通用、page-type-aware 快速抽取器。
...
### 11.2 rs-trafilatura 生产集成方式

生产平台优先直连 `rs-trafilatura-python` binding，而不是把 Crawl4AI `RsTrafilaturaStrategy` 当唯一入口：

```text
HTTP raw bytes
 -> RsTrafilaturaDirectAdapter
 -> extract_bytes(raw_bytes, explicit options)

Browser rendered DOM
 -> RsTrafilaturaDirectAdapter
 -> extract(rendered_html, explicit options)
```

显式选项至少包括：

```text
page_type override（可选）
favor_precision / favor_recall
include_tables=true
include_images=true（按站点策略）
include_links=true
include_comments（按 page type / site rule）
output_markdown=false
```

正式候选优先消费底层 binding 可提供的 `content_html`、`classification_confidence`、`images`、metadata 和正文，而不是只消费兼容 Adapter 裁剪后的 dict。

Crawl4AI 的 `RsTrafilaturaStrategy` 可保留用于简单场景、内联验证和兼容测试，但其 capability 必须单独登记，不能假设与 direct adapter 等价。

### 11.3 完整 HTML、编码与 raw bytes

rs-trafilatura 需要完整文档，不应在正文抽取前 chunk。HTTP snapshot 优先用 `extract_bytes()`，让抽取组件有机会基于原字节处理编码；Browser DOM 已是 Unicode，则使用 `extract()`。

这保证旧博客错误 charset、历史模板或未来 decoder 更新时，可以从同一 raw snapshot 重新得到可复现结果。

### 11.4 CPU/GIL 与 Runtime 隔离

`asyncio.to_thread()` 的价值是避免同步函数直接阻塞 event loop，但不能被当成多核 CPU 并行的架构保证。当前第三方 Python/Rust binding 是否在某一版本显式释放解释器锁必须以固定源码和 benchmark 为准。

因此默认策略：

- Fetch Worker 与 Extract Worker 分进程/容器。
- Extract 默认使用独立 worker process；按 CPU core 和真实 benchmark 设置并发。
- 每个 worker 对 in-flight input count 和总 HTML bytes 都有限制。
- 记录 CPU time、wall time、RSS 和 input/output bytes。
- 若未来版本确认安全释放解释器锁，再通过 A/B benchmark 决定线程池是否值得启用。
- 第三方 native extension 崩溃、内存峰值或畸形 HTML 超时不应影响 Fetch 主循环。

### 11.5 第三方质量分语义

rs-trafilatura 当前常规抽取结果的 `extraction_quality` 应按其具体 release 的语义登记，例如：

```text
raw_quality_kind = rs_trafilatura_heuristic_vX
```

不能把它默认等价为“经过统一校准的 ML F1 预测”。若第三方另有独立 ML quality API，也应作为不同 signal/version 管理。

因此禁止全局硬编码：

```text
raw_quality < 0.80 -> 某固定 fallback
```

阈值要在自有 golden corpus 上按站点、page_type、模板和 extractor release 校准。
...
- extractor 原始质量分及其 `raw_quality_kind`；
...
### 11.8 Shadow Extraction

probing、抽取器升级和规则发布前，在 golden snapshots 上双跑/多跑 Extractor，比较正文覆盖、boilerplate、代码/表格/链接/图片保真、metadata、page_type、CPU/wall time、RSS 和人工正确率。

生产正常流量只跑主 Extractor，质量失败才跑备用，避免永久双跑导致 CPU 成本翻倍。

### 11.9 Contract Tests

每个 `extractor_release` 上线前至少通过：

1. 完整 HTML 输入：包含正文、导航、footer、JSON-LD，确认未被错误 chunk。
2. 链接/图片：显式 include 配置后 Candidate 能得到平台需要的信息。
3. 编码：UTF-8、Windows-1252、ISO-8859-1、错误 charset；验证 raw-byte replay。
4. page-type override：站点规则强制 article/documentation 后行为可预测。
5. raw quality semantics：版本升级后不会把不同含义的分数混成同一指标。
6. Crawl4AI input contract：`input_format="html"` 的兼容路径仍获得完整 HTML。
7. 幂等 replay：同 snapshot + release + config 得到稳定 candidate hash。
8. 资源上限：超大 HTML、深层 DOM、重复节点不会造成无界 CPU/RSS。

Contract test 失败的 release 只能保持 `testing`，不能切换到 `active`。
...
ETag、Last-Modified、304、raw body hash、content/Markdown hash 独立保存。Provider 本轮没发现变化不等于页面正文没变化。
...
网络 I/O 与 CPU 抽取是不同资源池：

- HTTP Worker 按连接/域限流。
- Browser Worker 按浏览器 slot/RSS 限流。
- Extract Worker 按 process、CPU、RSS、输入 bytes 限流。

建议保守起步：domain concurrency=1；HTTP Worker 每进程 20~50 slot；Browser 4~8；Sitemap child 2~4；Extract 从每进程 1~2 个 in-flight 开始，以多进程 benchmark 决定总并发，不以 `to_thread()` 数量作为容量指标。

Frontier 每次 lease 20~100 个执行项；Discovery UPSERT 500~2000/批。采用 weighted fair queue，日常增量高于历史 backfill，单个大站不能垄断全部 Worker。

当 PG/S3 延迟、Redis pending、Extract backlog、Browser RSS、Extract CPU/RSS 或 object-store throughput 异常时，Scheduler 自动降低上游 Fetch/Discovery 速率。
...
规则 draft/testing/publish/rollback；golden URL/snapshot 批测；selector 命中率；before/after Markdown diff；canonical/metadata 变化；Capture Profile、Extraction Profile、Extractor Release 变化预览。

### 20.5 Extraction 视图

展示每次 attempt 的 extractor/release/adapter version、input kind/hash、page_type、classification confidence、raw quality kind/score、platform quality、gate reason、主/备用 candidate diff、CPU/wall time、RSS、Browser escalation 原因以及 shadow extraction 结果。

### 20.6 Extractor Releases

Web 管理：

- release source ref / artifact digest；
- capability matrix；
- runtime kind；
- raw quality semantics；
- contract-test 结果；
- golden regression；
- testing/active/retired 状态；
- canary、切换和 rollback。
...
显示 domain limiter wait、HTTP/Browser/Extract active slots、Browser RSS、Extract CPU/RSS/latency、provider 请求量/字节、budget exhaustion、soft-404、空正文、重复正文和质量 drift。
...
- per extractor release/page_type pass rate；
- alternate extractor fallback；
- candidate disagreement；
- raw/platform quality 分布和 drift；
...
Redis lag/pending、DB lease/reclaim、PG/S3 latency、HTTP status/latency、Browser slot/RSS、Extract process active、CPU/RSS/wall time、input bytes、domain limiter wait、egress deny、bytes/request budget 和 micro-batch duration。
...
自动检测：正文过短/为空、导航模板比例高、登录/验证码、soft-404、title/date 异常、canonical host 异常、metadata 冲突、`<base href>` 异常、重复正文、Markdown 结构异常、无意义版本 churn、selector zero-result、extractor disagreement、extractor capability mismatch、raw quality semantic change 和 extractor release quality drift。

质量失败时按原因分流：备用 Extractor、Browser、规则切换或人工 review；不直接覆盖 current_version。

## 25. Security / Reliability Control Matrix
...
extract:
  ...
  max_input_bytes_per_attempt: 10485760
  shadow_on_probing:
    - rs_trafilatura_direct
    - crawl4ai_generic
  rs_trafilatura_direct:
    prefer_raw_bytes: true
  ...
  quality_gate:
    ...
    raw_quality_kind_required: true
...
Quality threshold 的具体数值不写成全局固定常量，应在 golden corpus 上按站点/page_type/extractor release 校准后由规则版本发布。
...
优先实现 Feed/Sitemap、Frontier、多来源 attribution、outbox、conditional HTTP、raw-byte snapshot、Browser fallback、Capture Snapshot、`RsTrafilaturaDirectAdapter`、多 Extractor + Quality Gate、Article IR、Markdown、Web CRUD/任务/错误/Extraction 视图、增量 checkpoint、Egress、预算、日志 sanitizer 和 metadata provenance。

Extract Worker 与 HTTP Worker 独立部署，优先使用 process 隔离，避免把第三方同步/native 抽取直接塞入 Fetch asyncio 主循环。

### Phase 2：历史覆盖与质量强化

Common Crawl/Wayback、DomainMapper probing、soft-404、归档页 Provider、golden/shadow Workbench、Extractor Release/Capability Registry、contract-test pipeline、extractor drift 告警、metadata conflict review 和完整 Control Matrix。
...
- Extract 先按多进程基准，从每进程 1 个 in-flight、每节点 2~4 个 process 起步；
- 不把 `asyncio.to_thread()` worker 数直接作为 Extract 吞吐容量；
- Browser memory threshold = 70%~80%；
- Frontier lease = 100~1000；
- Discovery UPSERT = 500~2000/批。

容量规划按 HTTP/Browser 比例、Extract CPU/RSS、每页字节、平均响应时间、域 politeness、历史 URL 总量分别测量，不能简单按“100 个站耗时”线性外推。
...
```text
New extractor release/rule draft
 -> capability + contract tests
 -> golden snapshots shadow replay
 -> quality/resource/diff regression
 -> canary
 -> publish release/version
 -> enqueue re-extract from saved raw snapshots
...
- HTTP 原始 bytes 可持久化，后续能够 `re-extract without refetch`。
- HTTP raw bytes 与 Browser rendered DOM 输入类型明确区分。
- 支持 `RsTrafilaturaDirectAdapter`，HTTP raw 优先走 `extract_bytes()`，Browser DOM 走字符串 `extract()`。
- include links/images/tables/comments 等关键能力由显式配置和 capability registry 管理。
...
- Extractor Release 上线前必须通过 capability/contract tests 和 golden regression。
- golden snapshots 可进行 shadow extraction 和批量回归。
- Metadata 候选、最终值和 provenance 可追溯。
- Web 可查看 extraction candidate diff、raw quality kind、质量分和 gate reason。
...
- Extract CPU/内存资源独立于网络 async 并发。
- 单个大站不能垄断全部资源。
- 抽取器升级可基于历史 raw snapshot 离线重跑。
...
- 第三方 raw quality 必须携带 `raw_quality_kind`，不直接作为全局固定阈值。
- extractor release 上线后的质量 drift、资源 drift 和 capability drift 能自动告警。
- 同一 raw snapshot + extractor release + config 可稳定 replay。
...
- Extract native extension/CPU/RSS 异常被独立 worker/process 隔离，不拖垮 Web/API/Fetch。
- 不依赖“`to_thread()` 必然多核并行”这一未经验证的假设；所有执行模型都有 benchmark、容量指标和回滚策略。
