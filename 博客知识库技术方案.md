...
3. 普通站点不编写独立爬虫；站点差异依次通过 Generic Profile、Platform Profile、Site Override、Custom Adapter 解决。
...
7. **所有影响结果的 release/version 可追溯**：Platform Profile、Rule、Extraction Contract、Canonicalizer、Fetcher、Capture Profile、Extractor、Quality Gate、Serializer、Security Policy。
...
- **控制面**：站点、Platform Profile、规则、Extraction Contract、计划、权限、任务、发布、回滚、预算。
...
| Web | React / Next.js | 站点、Profile、规则、Contract、任务、diff、质量、安全、预算、Topology、Search |
...
| 监控 | Prometheus + Grafana + OpenTelemetry | 系统 SLO + 业务质量 + Profile/Contract drift + 索引新鲜度 |
...
- 不允许把 Crawl4AI Markdown 包回 HTML 后冒充原始页面输入。
...
Retry：429 尊重 Retry-After；5xx/timeout 指数退避 + jitter；security/robots/access_restricted 不无限重试；parse/extract 优先基于 snapshot replay；index/embedding 失败基于已发布 article_version 重试，不重新抓源站。

取消只写 `cancel_requested=true`；Scheduler 停发新 item，worker 在领取和阶段边界检查，`finally` 释放 domain lease、DB lease 和 Browser context。

正文、metadata、asset、enrichment、indexing 分别建状态；单张图片、摘要或 embedding 失败不能把正文成功伪装成整篇失败。
...
网络 I/O、Browser、CPU 抽取和 Embedding/Index 是不同资源池。
...
采用 weighted fair queue：日常增量高于历史 backfill，单个大站不能垄断全部 Worker。PG/S3 延迟、Redis pending、Extract backlog、Browser RSS、CPU/RSS、object-store throughput 或 Search backend 延迟异常时，Scheduler 自动降低对应上游速度。

Knowledge Access Plane 有独立低优先级队列和预算；大规模 re-embedding 不得挤占新增文章的抓取与发布，也不得触发源站 refetch。
...
全量 backfill、单站增量、reconciliation、source probing、单 URL 重抓、re-extract without refetch、asset retry、export、enrichment、index/reindex、cancel/pause/resume、job/item 进度和重试。

### 18.4 Rule / Profile / Contract / Golden Workbench
...
显示 domain limiter wait、HTTP/Browser/Extract/Index slots、Browser RSS、Provider 请求量/字节/费用、scroll count、budget exhaustion、SSRF deny、robots deny、access restricted、异常 active content、Embedding 调用量/费用和 Search backend 压力。
...
Redis lag/pending、DB lease/reclaim、PG/S3 latency、HTTP status/latency、Browser slot/RSS/process recycle、scroll/action duration、Extract CPU/RSS/wall time、domain limiter wait、egress deny、bytes/request budget、Embedding API latency/error/cost、Search backend latency/QPS/indexing throughput。

### 20.3 Trace

`job_id -> discovery_run -> frontier_url/discovery_edge -> fetch_snapshot -> extraction_attempt -> article_version -> asset/enrichment -> search_chunk/index_generation` 全链路携带 correlation/trace id。
...
PG 保存 metadata/object key，不存大正文。Search backend 只存检索所需的派生文本、向量与 metadata，不保存无法从 canonical artifact 重建的唯一信息。

导出 job：路径 segment 规范化、禁止 `..`/绝对路径、处理 Windows 保留名和长度、拒绝 symlink 逃逸、临时文件 + fsync + atomic rename；manifest 保存 article/version/source/hash/local path。导出路径不能由 Discovery Graph parent title 拼接产生。
...
### Phase 4：按瓶颈引入 Kubernetes

需要多机 Browser/Extract/Index pool、弹性扩容、节点隔离或高可用时迁移 K8s。1000 个站点本身不要求第一天上 K8s。
...
容量规划按 HTTP/Browser 比例、滚动归档比例、Extract CPU/RSS、每页字节、平均响应时间、域 politeness、历史 URL 总量、Discovery Edge 增长率、chunk 数/文章、Embedding 成本和 Search QPS 分别测量。
...
### 25.3 Profile/Rule/Contract/Extractor 升级
...
### 25.7 Knowledge Access / Index Generation
...
- DB/队列仍由 transactional outbox 保证，不因引入 Search/Embedding 改回 DB + queue 非原子双写。

## 27. 站点拓扑与 Discovery Graph 设计
...
## 28. Knowledge Access Plane：全文检索、向量检索与只读 MCP

### 28.1 Canonical 与派生索引严格解耦

只有 `article_version` 成功发布后才发出索引事件。索引面不直接消费“刚抓到的 HTML”或未通过 Quality Gate 的 candidate；否则会出现正式 Markdown 与检索内容不一致。
...
中文技术语料建议对 title、heading、code symbol、tag、domain/site 提供 field boost/filters，同时保留原文 chunk，避免只索引 LLM 摘要。

### 28.4 Index Generation 生命周期
...
起步阶段优先减少组件数量：PostgreSQL FTS + pgvector 足以验证检索体验和 chunk 模型。只有在数据量、向量规模、召回延迟或 QPS 成为瓶颈时再迁移独立 Search/Vector backend。

无论选用哪种后端，都必须满足：

- generation/version 可表达；
- article_version 可追溯；
- bulk rebuild / delete-by-generation；
- tenant/site filter 能下推；
- 索引损坏可从 PG/S3 全量重建；
- backend 不承担抓取 job/frontier/article current version 等业务状态。
