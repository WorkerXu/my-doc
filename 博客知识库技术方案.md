...
```text
runtime_role            # http/discovery/browser/extract 等
source_commit_sha
...
Web 能定位任一 URL 的发现来源、Sitemap 父子关系、Admission 决策、Egress/DNS/redirect 决策链、页面类型、priority score、抓取 attempt、snapshot、抽取 candidate、质量结果、文章版本和发布状态；关键步骤均有 metrics、logs、trace 和审计记录。
