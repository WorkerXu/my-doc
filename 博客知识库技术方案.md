...
3. 支持 `FULL_BACKFILL`、`INCREMENTAL`、`RECONCILE`、`TARGETED_FETCH`、`REPROCESS`、`ASSET_REPAIR`、`REINDEX`、`SEARCH_GAP`。
...
21. Platform Adapter / Site Profile / Crawler Engine Release / Search Provider Release 发布前必须通过 Contract Test、Golden Corpus、Capability Test 和最近 Snapshot 的 shadow replay。
...
31. 搜索响应的“Provider 报告总量”“本页返回量”“客户端截断后数量”“规范化去重数量”“站内有效数量”“新缺口数量”必须分别计数，禁止复用同一个 `number_of_results` 字段混淆语义。
32. Search Worker 与 HTTP Worker 都必须复用长生命周期 async client/连接池；不能按每次 query/URL 新建完整客户端或浏览器 runtime。
...
11. `max_depth/max_pages/max_runtime/max_urls/max_results` 是预算，不是 Coverage 证据。
...
HTTP、Browser、PDF/OCR、Extraction、PostProcess、Quality、Identity 分为独立 Worker Pool。Browser Worker 内部通过 Engine Adapter 调用 Crawl4AI/Playwright；Search Gap Worker 内部通过 Search Adapter 调用 SearXNG。

### 4.4 Projection Plane

从 Accepted Document Version 构建 Markdown、Current Projection、Chunk、Embedding、Search、AI Artifact 和 Delivery；从 Search Evidence 构建 Web/MCP 紧凑结果 Projection。
...
第三方依赖必须进入 Release：Crawl4AI、Playwright、FastMCP、httpx、tldextract 等版本变化可能改变 strategy、Host/Origin 安全策略、返回 shape 或 timeout 行为，不能只记录业务代码版本。
...
SearXNG 必须启用 JSON 输出。认证使用 secret ref，不能把用户名/密码直接放入 Site Profile。`result_projection_fields` 只控制 UI/MCP/Export 输出，不改变原始响应保存。
...
### 10.6 连接池、并发与紧凑 Projection

Search Gap Worker 按 Search Provider Release 复用长生命周期 `httpx.AsyncClient`，配置 connection limit、keep-alive、connect/read/write/pool timeout、Provider 级 semaphore/token bucket 和 circuit breaker。像交互式 MCP 工具那样每次 query 使用 `async with new AsyncClient()` 只适合低频调用，不适合批量 Gap Reconcile。

完整 SearXNG 响应先落对象存储并计算 hash，再做：

```text
Raw Search Response
 -> Schema Validation
 -> Search Evidence Normalizer
 -> URL Normalize / Scope Gate / Dedup
 -> Gap Candidate
 -> UI/MCP Result Projection
```

UI/MCP Projection 可使用字段白名单减少 token 和带宽，但必须能通过 `query_id + raw_response_snapshot_id` 回溯完整证据。
...
若未来确需抓取已获授权的登录态内容，Playwright `storage_state` 必须作为加密 Secret Artifact 管理，记录 site/tenant scope、创建时间、过期时间、版本和撤销状态；不得直接进入普通 Profile、日志、Snapshot 或 MCP 输出。Browser Context 必须按租户/站点隔离，禁止跨站复用登录态。
...
Representation Type 明确区分 `HTTP_BYTES / HTML_SOURCE / HTML_RENDERED / DOM_SERIALIZED / CLEANED_HTML / MARKDOWN_CANDIDATE / JSON_API / FEED_XML / TEXT_PLAIN / PDF_BYTES / OCR_TEXT / EXTRACTION_CANDIDATE / CANONICAL_IR / MARKDOWN_PROJECTION / ASSET_BYTES / SEARCH_RESPONSE_JSON`。
...
timeout/cancel 必须向 HTTP request、Search request、Browser Page/Context、第三方 crawler coroutine 传播，并持久化 `*_DEADLINE_EXCEEDED` Outcome，释放 lease、连接和页面资源。重试只针对可恢复错误；幂等写入防止重复版本。
...
浏览器、HTTP、Search Gap、OCR、Embedding 分独立队列。根据 queue lag、active pages、active search connections、CPU/RAM、429 比率、site/provider budget 扩缩容。单站点并发/速率上限优先于全局吞吐。
...
- 分开展示 `provider_reported_total / returned / unique / in_scope / new_gap`，避免把截断条数误认为搜索总量；
...
MCP/CLI 适合 `targeted_fetch / diagnose_site / search_gap / get_run_status / reprocess_snapshot`，不直接承载长时间 FULL_BACKFILL/INCREMENTAL。
...
Trace 贯穿 `run_id -> task_id -> query_id/candidate_id -> snapshot_id -> document_version_id -> projection_id`。日志禁止包含 Cookie/Authorization/storage_state。
...
- Search 原始证据与 Projection 可通过 query_id 完整回溯。

## 27. 测试与发布门禁

1. Unit Test：URL normalize、scope、checkpoint、dedup evidence、hash、identity、Search count semantics。
...
5. Search Provider Test：SearXNG JSON、鉴权、分页、字段 provenance、429/5xx、超时、去重、字段 Projection、截断前后计数语义、连接复用。
...
9. Dependency Capability Test：升级 Crawl4AI/FastMCP/httpx/Playwright 后验证 Host/Origin guard、strategy/stream、return shape、timeout 和连接池行为。
...
Coverage Evidence、SearXNG Search Gap Provider、迁移/旧 URL 诊断、Provider 差集、Search Gap Web 视图、Search count semantics、Runtime Attestation、Crawler Quirk Registry、Capability Test。
...
11. Search 原始 JSON 永远先于字段 Projection 保存；UI/MCP 字段白名单不会造成 provenance 丢失。
12. `provider_reported_total / returned / normalized_unique / in_scope / new_gap` 可独立审计，客户端截断不会改写 Provider 原始总量语义。
13. Crawl4AI/FastMCP/httpx 等依赖升级后必须通过 Capability Test，运行时事实与声明配置不一致时阻止 Release 发布。