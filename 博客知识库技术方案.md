...
4. 将“动态页面动作”和“结构化抽取规则”拆开：Browser Action Plan 只负责页面就绪、翻页、滚动、点击等受控动作；Extraction Contract 负责从 HTML/Rendered DOM/JSON 中确定性提取链接、Metadata 和正文候选。
5. 支持 HTTP-first、Browser 按需升级；动态下拉、加载更多、无限滚动使用有界、版本化、可审计的 Browser Action Plan。
6. 支持 ETag、Last-Modified、Sitemap lastmod、Feed GUID/cursor、内容 hash、Provider checkpoint 和重叠窗口驱动的增量同步。
7. 支持断点续跑、幂等、失败重试、指数退避、暂停/恢复、durable cancel、规则回放、离线重抽取和版本回滚。
8. 原始 HTTP 响应、Browser rendered DOM、抽取候选、Metadata candidate、Article IR、Markdown、规则版本和质量结果均可追溯。
9. 提供 Web 管理端完成站点接入、栏目/发现端点管理、Extraction Contract/Action Plan 发布、动态动作 dry-run、抓取进度、错误诊断、质量审核、版本 diff、重复组、资源监控、搜索评测和发布管理。
10. 搜索、Embedding、Reranker、主题分类、摘要和外部 RAG 都是可重建派生层，故障不得迫使系统重新抓取源站。
11. 任何 transport 都统一经过 robots、访问政策、SSRF/Egress、安全预算和域级限流，不以绕过登录、验证码、付费墙或明确访问控制为目标。
...
5. **页面动作与内容抽取分离。** Browser Action Plan 不负责字段语义，Extraction Contract 不负责无限执行页面动作。
6. **动态浏览动作必须有界。** scroll/click/pagination 都必须有最大次数、时间、链接数和无增长停止条件。
7. **抓取快照不可变。** 抽取规则变化优先对旧 snapshot 离线 replay，而不是重新请求源站。
8. **文章身份与 URL 分离。** redirect、canonical、镜像、迁移不等同于新文章。
9. **版本追加优先。** `article_version` 不覆盖旧版本，current 只是指针。
10. **强去重与近似重复分离。** content hash 可确认强重复；MinHash/SimHash 只产生候选组。
11. **流程状态与内容完整度分离。** `done` 不代表一定有全文。
12. **published_at 与 observed_at 分离。** 不知道真实发布时间时宁可为空。
13. **消费时间窗口与采集状态分离。** “昨天/最近 24 小时”属于 Search/Export 查询，不写进 crawler 状态机。
14. **控制面不执行长任务。** UI/API/Cron 只创建 durable job。
15. **所有跨 PG 与队列的写入通过 Transactional Outbox。**
16. **所有影响结果的组件都版本化。** Profile、Extraction Contract、Action Plan、Canonicalizer、Fetcher、Extractor、Normalizer、Resolver、Quality Gate、Serializer、Chunker、Embedding、Reranker、Publish Adapter 均有 release id。
17. **LLM 只做候选或派生能力。** 不让 LLM 静默改写 canonical 正文或无证据决定 metadata。
18. **所有结构重写在 DOM/IR/AST 层完成。** 正则只做字段级清洗。
19. **desired state 与 observed state 分离。** 搜索索引和外部知识库通过 reconciliation 最终收敛。
20. **阻塞 I/O 必须隔离。** async Worker 中禁止同步网络 SDK 直接阻塞 event loop。
21. **配置生命周期与内容生命周期分离。** 禁用站点/栏目默认不删除历史内容。
22. **数据库 schema 显式迁移。** 使用 Alembic 等 migration 工具。
23. **增量正确性依赖 Provider 连续性，而不是固定轮询。** windowed Provider 必须持久化 checkpoint、水位、重叠窗口和断档状态。
...
- **控制面**：站点、发现端点、Profile、Extraction Contract、Action Plan、规则、计划、预算、权限、任务、发布、回滚。
...
- **抽取面**：Extraction Contract、正文候选、Metadata candidate、Normalizer、Resolver、Quality Gate、Article IR、Markdown Serializer。
...
| Web | React / Next.js | 站点、端点、抽取契约、动作计划、质量、搜索、发布管理 |
...
| 大对象 | S3 / MinIO | raw response、rendered DOM、IR、Markdown、附件、调试快照 |
| HTTP | httpx / aiohttp | 长生命周期连接池、条件请求、流式响应 |
| Browser | Playwright + Crawl4AI Adapter | JS 页面、动态列表、rendered DOM、结构化抽取 |
| HTML/正文 | DOM parser + Trafilatura/Readability/Crawl4AI Adapter | 版本化 Extraction Contract + 多候选抽取 |
...
- `locale / region`
- `profile_release_id`
- `discovery_contract_release_id` 可空
- `article_contract_release_id` 可空
- `action_plan_release_id` 可空
- `sync_policy_id`
- `enabled`
- `created_at / updated_at`

一个站点可以同时拥有多个 Sitemap、Feed、分类归档、年份归档、动态栏目和 API，各端点独立同步。同一 Source 的“列表链接发现契约”和“文章详情抽取契约”允许不同 release，避免把两种页面结构揉成一个规则。
...
- 默认 discovery/article contract 引用；
- HTTP/Browser 策略；
- canonicalization 规则；
...
Profile 负责“默认策略与能力”，具体 DOM 字段规则优先进入 Extraction Contract。

### 6.3.1 `extraction_contract_release`

Extraction Contract 把稳定站点的 CSS/JSON 字段抽取从业务代码中剥离出来，作为可测试、可版本化、可回放的确定性配置。
...
- `base_selector` 可空
- `fields_schema`：字段树，至少支持 `name/selector/type/attribute/required`、list/nested_list
- `link_selector / link_attribute`，用于 discovery contract
- `allow_url_pattern / deny_url_pattern`
- `transform_pipeline`：仅允许白名单 transform，如 trim、date_parse、url_join、normalize_space
- `readiness_predicate`：selector/min_item_count/dom_stable_ms 等声明式条件
- `adapter_type`：dom_json_css/readability/trafilatura/crawl4ai/custom-reviewed
- `test_snapshot_ids`
- `expected_field_hit_rate / expected_min_items`
- `schema_version`
- `release_state`：draft/canary/published/rolled_back
- `created_at / created_by`

原则：

1. discovery contract 只负责从给定快照提取链接/游标/栏目证据，不执行无限滚动；
2. article contract 只负责字段和正文区域抽取，不负责页面导航；
3. 任意 selector/字段变化必须生成新 release；
4. 发布前必须对固定 snapshot fixture 做 dry-run 和回归比较；
5. 确定性 contract 失败后可以进入通用 extractor 候选链，但不得静默覆盖 provenance。
...
无法用 DSL 表达的少数站点再使用受审计的 Custom Adapter。Action Plan 只产生“可供 Contract 处理的页面状态/快照”，不直接承担 canonical 内容抽取。
...
- `debug_artifact_manifest_key` 可空：采样截图、网络 trace、console 摘要等，仅用于诊断
- `body_hash`
- `fetcher_release_id`
...
调试截图/网络 trace 不应默认全量永久保存，按失败、抽样或规则调试策略留存，避免对象存储失控。
...
- `markdown_object_key`
- `extraction_contract_release_id` 可空
...
4. 生成候选 Platform Profile、Site Override、Discovery/Article Extraction Contract、Action Plan。
5. 抽样 20~100 个 URL 做 discovery/fetch/extract dry-run；Contract 必须在固定 snapshot fixture 上做字段命中率和输出 diff。
6. Web 管理端展示 URL 命中率、正文质量、metadata 完整度、字段 provenance、Browser 升级率、动作 trace 和异常样本。
7. 管理员发布 release。
...
Profile、Source Override、Extraction Contract、Action Plan 都必须可 diff、灰度发布和回滚。

### 7.3 规则变更的最小重放

- Extraction Contract selector/字段改动：只重抽取已有 snapshot；
...
- Action Plan 改动：只影响后续动态 discovery/capture；若页面动作决定能否获得完整 DOM，则按影响范围补抓，不把所有旧文章一律重抓。
...
优先使用 `wait_for/min_item_count/dom_stable` 等可验证就绪条件，固定 `sleep` 只是兜底，不应成为页面正确性的唯一依据。
...
- 按策略采样的 screenshot/network debug artifact。

### 9.4 优先识别底层 API

如果“加载更多”实际调用公开稳定的 JSON/XHR/API，应优先把 API 注册为 provider；Browser 只作为发现和验证手段。浏览器网络观测可辅助发现 API，但不能自动把带敏感 token、登录态或不稳定内部接口长期固化为 provider。
...
- Contract 在 response HTML 中不满足 required 字段，但在 rendered DOM 规则上可满足；
...
| selector/contract miss | 进入抽取诊断，不盲目 Browser 重试 |
...
Browser process 可长生命周期，但 Browser Context 必须按安全边界管理：默认不跨站点复用；同站点需要保留公开会话状态时，可在明确的 `site_id + session_policy_release` 范围内短时复用，并设置最大任务数/最大年龄，避免 Cookie、Storage、Service Worker 在不同站点间泄漏。
...
### 13.2 Extraction Contract 执行模型

固定流程：

```text
Snapshot
 -> 选择 input_representation(response_html/rendered_dom/json)
 -> 执行 Extraction Contract release
 -> 生成字段/链接候选 + provenance
 -> required/数量/类型校验
 -> 通用 Extractor 候选链（需要时）
 -> Resolver / Quality Gate
```

Contract 的结果不是不可质疑的最终真相，而是高确定性的候选。每个字段至少记录：
...
对于列表页，Contract 输出的是 URL/cursor/category 等 discovery candidate；对于详情页，Contract 输出 title/author/time/content region 等 article candidate。两者分别版本化。
...
- Contract required 字段命中率；
- Contract list/item 数是否低于 fixture/历史基线；
- 正文字符数/段落数；
- 标题与正文相关性；
- 导航链接密度；
- boilerplate 比例；
...
### 17.2 Extraction Contract

- 分开管理 Discovery Contract 与 Article Contract；
- JSON/CSS schema 可视化编辑；
- 字段 required/type/attribute/transform 白名单校验；
- 固定 snapshot fixture dry-run；
- release diff、canary、回滚；
- 展示字段命中率、缺失字段、原始 DOM path 和 transform provenance；
- 一键对旧 snapshot replay，比较新旧 Article IR/Metadata diff；
- selector drift 与字段命中率基线告警。

### 17.3 Browser Action Plan

- 可视化编辑动作 DSL；
- dry-run；
- 展示每轮新增链接数；
- readiness predicate 结果；
- scroll/click 次数；
- stop reason；
- rendered DOM；
- 失败/抽样截图和网络诊断；
...
- Contract candidate/metadata candidate/Resolver 证据；
- 版本 diff；
...
至少 viewer/operator/admin；站点规则发布、Extraction Contract 发布、Action Plan 发布、删除、Profile 回滚、手工重抓、Knowledge Sink 变更、Delete Job、schema migration 必须写 audit log。
...
- Discovery Extract：`snapshot_id + discovery_contract_release`
- Article Extract：`snapshot_id + article_contract_release + extractor_release`
...
Debug Storage ≈ sampled_or_failed_browser_runs × avg_debug_artifact_size × retention
```

正式导入前用 20~50 个代表站点测 p50/p95 HTML、Markdown、asset、Browser 升级率、Contract 命中率和版本变化率，再决定生命周期策略。
...
- readiness predicate 失败率；
- Contract required-field hit rate；
- Contract item count 与历史基线偏差；
...
同一 Profile/Extraction Contract 家族的多个站点 selector/required-field 命中率同时下降时产生“平台模板漂移”告警，而不是产生大量独立站点故障。
...
discovery_run
 ...
 -> discovery_contract_run 可选
 -> fetch_task
 -> fetch_attempt
 -> snapshot
 -> article_contract/extraction_run
 -> article_version
 -> index/publish
```
...
8. Extraction Contract 的 transform 只允许白名单操作；不得在 Web 中把任意 Python/JS 代码伪装成字段 transform。
9. Browser Context 默认不跨站点复用；会话状态有明确 scope、TTL 和审计。
10. 日志不记录 Cookie、Authorization、完整密钥和敏感 query 参数。
11. 第三方内容版权和再分发策略由站点 policy 记录；外部发布前再次检查 sink policy。
12. 无法合规访问时进入 blocked/manual，不把反爬挑战视为必须绕过的问题。
...
Profile、Extraction Contract、Action Plan、Extractor、Normalizer、Serializer 均可回滚 release；旧 snapshot 支持离线重放。Contract 回滚只改变后续/重放使用的 release，不删除旧 extraction candidate 和 provenance。
...
Create Site
 ...
 -> Profile / Discovery Contract / Article Contract / Action Plan Candidate
 -> Sample Dry Run on Fixed Snapshots
 -> Publish Releases
 -> Discovery Providers fan-out
 -> Browser Action if required
 -> Discovery Contract -> URL Candidates
 -> Durable URL Frontier
 -> Admission / Robots / Egress
 -> HTTP or Browser Fetch
 -> Immutable Snapshot
 -> Article Extraction Contract + Generic Extractor Candidates
 -> Metadata Resolve
 -> Article IR
 -> Quality Gate
 -> Markdown Serialize
...
Scheduler
 ...
 -> Article Contract + generic extractors + quality
...
```text
Publish Extraction Contract / Extractor / Profile Release
 -> select affected snapshots
 -> re-extract without refetch
 -> compare field hit rate + Article IR + metadata diff
 -> canary
 -> publish new article versions only if canonical content materially changed
```
...
- 版本化 Discovery/Article Extraction Contract；
- Contract fixture dry-run 与字段 provenance；
...
- Contract canary/批量 replay/selector drift；
- sampled screenshot/network debug artifact；
- distributed domain limiter；
- weighted fair scheduler；
- Scheduler HA；
- Provider gap detection + 自动 reconciliation；
- Browser generation/context/resource guard；
...
- Discovery Contract 能在固定 snapshot 上稳定重放并保留链接 provenance；
...
- readiness predicate 可验证且固定 delay 不是唯一正确性条件；
- action trace 可从 Web 查看；
- 同一文章来自多个栏目时保留多条 discovery edge；
- Browser 失败不会阻塞 HTTP Worker；
- Browser Context 不跨站点泄漏会话状态。

### 27.4 Markdown 与质量

- Article Contract required 字段命中率可观测；
- selector/Contract 升级可对旧 snapshot 离线 replay；
- 标题、正文、代码块、表格、图片结构稳定；
...
- Browser RSS/page/context 泄漏可触发 draining/recycle；
- 下游变慢产生 backpressure；
- event-loop lag 超阈值收缩本地并发；
- discovered/admitted/snapshot/contract-candidate/published 指标可对账。

## 28. 最终推荐

核心组合：

**FastAPI + Next.js + PostgreSQL + Alembic + Transactional Outbox + Redis Streams + S3/MinIO + asyncio HTTP Worker + 独立 Playwright/Crawl4AI Browser Worker Pool + `site_source/discovery_endpoint` 多端点发现模型 + 版本化 Discovery/Article Extraction Contract + 版本化 Browser Action Plan + DOM/Article IR 抽取层 + 版本化 Markdown Serializer + PostgreSQL/OpenSearch Search Plane。**

对于 1000 个技术博客，关键不是选择一个“最强爬虫库”，而是把站点、栏目/发现端点、Provider 连续性、durable frontier、抓取 attempt、HTTP/Browser 资源治理、不可变快照、抽取契约、文章身份、版本、质量、索引、发布和 Web 运维拆成独立且可重放的层。

动态站点尤其要坚持“动作与抽取分离”：Action Plan 负责把页面推进到可提取状态，Extraction Contract 负责从确定快照中产生可验证候选。这样列表页和详情页可以分别配置、分别测试、分别回滚；selector 变化可以离线 replay；固定 sleep、任意 JS 和每站点硬编码类不会逐渐演化成不可维护的 1000 份爬虫脚本。

这样后续新增网站、增加栏目、替换 Crawl4AI、替换 HTTP 库、增加 Parser、修改动态动作规则、迁移搜索后端或接入新的知识库，都可以在已有 durable state 和 immutable snapshot 上演进，而不重做整条链路，也不会丢失历史覆盖证据、增量连续性、失败尝试、抽取 provenance 和文章版本。