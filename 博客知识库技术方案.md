...
`transport_outcome=success` 不代表 `content_outcome=accepted`。例如 200 挑战页、空正文、只返回导航都属于正文失败。
...
### 15.3 Artifact Representation Contract
...
基础技术指标：queue lag、latency、timeout、retry、browser crash、DB/Redis/S3 error。
...
验证每个 Adapter 输入输出 schema、错误分类、timeout、取消传播和 provenance。
...
1. cache command/connect 有严格 timeout；
...
只有适合升级的错误才进入下一 route。例如 `JS_SHELL` 可从 HTTP 升 Browser；`ROBOTS_BLOCKED`/`POLICY_BLOCKED` 不升级；`RATE_LIMITED` 应延迟重试而不是立刻用更昂贵 Browser 猛打。
...
Browser Worker 以进程/Context 池复用 Chromium，但 Site Session、Cookie、Storage 隔离；Context 有最大页面数/寿命/内存阈值，异常自动回收。
...
8. Browser/GPU 独立预算；
9. S3 lifecycle：热/冷存储分层，但不可破坏审计要求。
...
Coverage 负责回答“应该有哪些文章”，Capability 负责回答“怎样抓更可靠、更便宜”，Snapshot/IR/Version 负责回答“当时抓到了什么、如何解释”，Projection 负责回答“怎样给人和 AI 使用”。四者彼此解耦但通过可追溯 Artifact 串联，才能同时满足全量历史、长期增量、低成本扩展、Web 运维和知识库可重建性。
