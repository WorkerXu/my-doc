...
| 状态库 | PostgreSQL | durable frontier、任务、attempt、版本、规则、审计 |
...
`discovery_run -> fetch_task -> fetch_attempt -> snapshot -> extraction_run -> article_version -> enrichment/index/publish`
...
对于 1000 个博客站点，最重要的不是选择一个“更强的爬虫库”，而是把站点发现、durable frontier、attempt 历史、抓取策略、HTTP/Browser 资源治理、不可变快照、抽取版本、身份/去重、增量状态机、schema 迁移、质量、派生 enrichment、索引、发布和 Web 运维拆开。

这样后续无论增加网站、替换 Crawl4AI、替换 HTTP 库、增加新的 Parser、迁移搜索后端、修改分类规则或接入外部知识库，都可以在已有不可变数据和 durable state 上重放与演进，而不重做整条链路，也不会丢失历史抓取、失败尝试和版本证据。