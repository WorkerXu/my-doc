...
3. 支持约 1000 个站点长期运行并持续扩站；普通博客不需要为每个站点开发独立爬虫。
...
- **Archive Full（可选）**：在合规前提下，通过 Common Crawl、Wayback 等公开历史索引补充已删除、迁移或不再被当前站点暴露的历史 URL/快照。

必须分别统计：发现过多少历史 URL、当前可访问多少、成功取得正文多少、仅有历史索引但无正文多少。
...
5. 生成 include/exclude、selector、identity/canonical、capture profile、metadata 候选规则。
...
每个 `DiscoveredUrl` 至少携带：URL、source/provider、discovered_at、lastmod/pubdate hint、depth、parent/evidence。Discovery worker 只做流式发现和批量 UPSERT，不在内存中保存全站 URL 列表。
...
- 同时支持 `<urlset>` 与 `<sitemapindex>`，按 XML local-name 处理 namespace。
- gzip 既看 URL/Content-Type，也检查 magic bytes；不能只依赖 `.gz` 后缀。
- child sitemap `<loc>` 允许相对 URL，但必须相对当前 Sitemap URL resolve 后重新执行 EgressPolicy。
- `<lastmod>` 作为调度提示保存，但不能作为正文必变的唯一证据。
- 不支持的根节点、坏 XML、子 Sitemap 失败均形成结构化错误；root run 可以 partial，不因单个 child 失败而丢掉已发现结果。
- 维护 child sitemap visited identity，检测 cycle；同一 child 多次出现只处理一次。
...
对 root/child 保存 Validator、last_success、status 和 checkpoint；预算耗尽使用 `partial/budget_exhausted`，保留已完成批次和续跑位置。
...
```text
platform/day -> site/day -> job -> provider/fetch -> request
```

关键指标：requests、bytes compressed/decoded、redirects、XML elements/URLs、Browser subrequests/bytes/wall-time、assets/article、asset requests、asset bytes、failures/timeouts。
...
URL 规范化需要版本化，至少处理：scheme/host 大小写、默认端口、fragment、query 参数规则、trailing slash、percent encoding、IDNA、tracking 参数；userinfo 直接拒绝。
...
跨域 canonical 默认不接受，除非 allowed alias 规则明确允许。`article_key = hash(site_id + stable_identity)`，不得由导出文件路径或发现顺序决定。
...
## 18. Metadata / Front Matter

Metadata 是独立的结构化抽取结果，不能把 YAML Front Matter 当真相源。

### 18.1 候选与 provenance

抽取来源可以包括：HTML `<title>`、`meta[name]`、OpenGraph `meta[property]`、`meta[http-equiv]`、`<link rel=canonical>`、JSON-LD、站点 selector 和启发式结果。每个候选保存：
...
重复 meta key 不用“最后一个覆盖前一个”的无痕字典语义，而是保留候选集合，再按版本化 precedence rule 选择最终值；冲突可在 Web 中追溯。
...
- max metadata field count；
- max key length；
- max value length；
- max total normalized metadata bytes；
- JSON-LD 单块和累计大小上限。

超过上限时记录 `metadata_truncated` 或 `metadata_budget_exhausted`，不能让异常 meta/JSON-LD 占满内存。

### 18.3 Canonical 与 URL 字段

canonical、OpenGraph URL、JSON-LD URL 等全部先按 `document_base_url` resolve，再经过 URL normalizer、allowed-host 和 EgressPolicy；不能因为它位于 `<head>` 就直接信任。

`requested_url`、`final_url`、`accepted_canonical_url`、`source_url` 必须语义分离，避免只用一个模糊字段覆盖不同含义。

### 18.4 Front Matter 导出

Front Matter 由已经归一化的 metadata 生成：

- 使用安全 YAML serializer，不允许任意对象 tag；
- key 顺序、Unicode、日期和空值格式由 serializer version 固定；
- 设置单值和总 Front Matter 大小上限，建议默认总量不超过 64 KiB；
- 超出时只截断导出视图，不修改 PG 中的结构化真值；
- 对无法完整导出的字段在 manifest/provenance 中保留状态。

## 19. Markdown 确定性与版本化

Serializer 必须固定：heading/list/code fence 风格、空行规则、link/image 格式、Front Matter key 顺序、Unicode/换行策略、table 规则。
...
URL hash 只能作为 source/cache key，不能冒充内容地址；同一二进制通过多个 CDN URL 出现时只保存一个 blob。
...
- 路径必须由稳定 article identity 决定，不能因发现顺序变化；
...
先实现：Feed/Sitemap、Frontier、Redis Streams + lease + outbox、HTTP + Browser fallback、capture snapshot、Markdown、Web CRUD/任务/错误、增量 checkpoint、Egress/预算/日志 sanitizer、metadata budget/provenance。
...
- metadata provenance/conflict review；
- 质量 SLO；
