...
适用范围：从约 1000 个技术博客网站抓取尽可能完整的历史文章，清洗为稳定 Markdown 形成长期知识库；后续持续增加站点，并支持长期增量同步、版本保留、Web 管理、搜索、审计、故障恢复与外部知识库发布。整体按“百万级文章、数百万到千万级 URL、站点高度异构、长期运行、可横向扩展、可恢复、可回放、可审计”设计。

## 1. 建设目标

1. 管理员只需录入博客根地址，系统自动探测 Sitemap、RSS/Atom/JSON Feed、归档页、分类/标签/作者页、平台 API、普通站内链接和动态列表，生成可审核的站点配置草稿。
2. 支持约 1000 个站点历史全量回灌，并可持续新增站点；新抓取引擎、新发现方式和新抽取器通过 Adapter/Provider 扩展，不修改核心状态机。
...
9. 周期计划、逻辑执行、后台任务和真实产物完全分离；计划是持久化业务配置，Scheduler 只是可重建触发器，实际抓取由 Worker 执行。
10. Web 管理端支持站点接入、覆盖检查、同步计划、任务管理、质量审核、版本 diff、错误诊断、搜索、成本和发布管理。
11. 每一次运行都有稳定 `crawl_run_id`；Worker 不得私自创建或切换运行上下文。
12. 核心业务写入使用类型化 Domain Writer；阶段完成必须以数据库状态和真实 COMPLETE artifact 对账，不能只相信 Worker 自报成功。

## 2. 核心原则

1. PostgreSQL 是业务状态真相源；S3/MinIO 是不可变大对象真相源。
2. Redis Streams 只做短期任务运输；状态先落 PostgreSQL，再通过 Transactional Outbox 投递。
3. URL 发现、准入、抓取、抽取、质量、索引、发布、计划与执行全部可版本化、可审计。
...
10. 在样本上运行 JSON-LD/meta、CSS/XPath、Trafilatura、Readability、Crawl4AI Markdown 等候选，输出质量证据。
...
## 8. Sitemap 与 Archive：持久化、可恢复、流式处理
...
## 9. Durable Frontier、URL Admission 与 Crawler Trap
...
## 16. HTTP-first、固定 fallback 与 Adapter
...
固定 fallback 顺序由 `fetch_profile_release` 定义，Worker 不能临时自由尝试无穷路径。
...
## 17. Snapshot、Artifact Commit 与 Run Artifact Manifest
...
4. PostgreSQL 事务写 artifact_object + fetch_snapshot + 状态 + event + outbox
5. COMMIT 后 snapshot=COMPLETE，对象不可变
6. 重试通过 idempotency key/hash 复用已有完整对象
7. Cleanup Worker 回收无引用的过期 staging object
```

`run_artifact_manifest` 保存每个 run/phase 的真实产物引用，例如：
...
## 18. Typed Domain Writer 与 Stage Finalizer
...
- 下游阶段只消费 Finalizer 返回的 canonical persisted set。

这能避免“Worker 日志说成功、Web 显示完成，但 Markdown/Snapshot 实际缺失”的状态漂移。

## 19. 抽取链、Article IR 与 Markdown
...
Dashboard 查询 PostgreSQL、Search 和 artifact metadata 中已提交事实。按钮只向 Control API 发显式命令；前端不持有任务状态或继续门禁。

## 25. Event Timeline 与 Web 实时进度
...
- `sync_schedule + crawl_run + background_task` 分离；
...
标题、发布时间、正文、代码块、表格按抽样集评估；规则升级可基于 snapshot replay；Markdown 不因文本扁平化破坏技术结构；低质量新版本不能覆盖高质量旧版本。
...
**持久化多源发现 + Durable Frontier + 持久化同步计划 + Append-only Crawl Run + HA Scheduler 只触发不执行 + Transactional Outbox + 可恢复后台任务 + HTTP-first + Browser 按证据升级 + 不可变 Snapshot + 原子 Artifact Commit + Run Artifact Manifest + Typed Domain Writer + Stage Finalizer Reconciliation + 多候选抽取 + Article IR + append-only 文章版本 + 增量/对账 + Web 管理。**

关键工程约束：

- Sitemap/Archive 是可恢复任务树；任何大小、条目数、深度和运行时间限制都只是 execution slice 安全预算，不能静默截断历史。
- `sync_schedule`、`crawl_run`、`background_task`、领域数据和 artifact 各司其职。
- Schedule 触发语义是业务数据，必须持久化 revision、timezone、DST、misfire/catch-up、`expected_fire_at` 和 `next_due_at`。
- Scheduler 到期只创建 run + event + outbox，不直接抓取。
...
- Dashboard 读取已提交事实并发出显式控制命令，不成为流程状态真相源。
- 内部关系使用稳定 ID，绝不使用标题、slug 或 URL path 尾段作为更新 key。
- 供应链通过 lock、SBOM、artifact hash、image digest、contract test 和 fault injection 固定。

该方案可从几十个站点平滑扩展到 1000+ 站点，并在持续增加新站点、新发现方式、新抓取引擎和新同步频率时保持核心状态机稳定，同时保证长期增量同步在 Scheduler 重启、停机补触发、DST 边界、Worker 崩溃、对象上传中断、消息重放和多副本部署下仍可恢复、可去重、可审计、可对账。
