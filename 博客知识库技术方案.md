...
8. 搜索、Embedding、Reranker、外部 RAG/知识库都是可重建派生层，故障不得迫使系统重新抓取源站。
...
13. **所有影响结果的组件都版本化。** Profile、Rule、Canonicalizer、Fetcher、Capture Profile、Extractor、Metadata Schema、Normalizer、Resolver、Quality Gate、Serializer、Chunker、Embedding、Reranker、Publish Adapter 均有 release id。
...
- **控制面**：站点、Profile、规则、计划、预算、权限、任务、发布、回滚。
- **发现面**：Source Discovery、URL Discovery、Discovery Graph、覆盖证据。
...
- **资源保护面**：Browser generation、RSS/page/context 监控、event-loop lag、worker drain。
...
| Worker | Python asyncio | I/O 型发现、抓取、抽取 |
| 状态库 | PostgreSQL | durable frontier、任务、版本、规则、审计 |
| 队列 | Redis Streams | 短期分发、consumer group；不做真相源 |
| 大对象 | S3 / MinIO | response bytes、DOM、Markdown、附件、派生 artifact |
| HTTP | httpx/aiohttp 等 | 条件请求、超时、重定向、压缩、连接池 |
...
不能以“队列为空”作为唯一完成条件。每个 Provider 需要记录 `exhausted / partial / blocked / error`；Full Backfill 只有所有必须 Provider 已达到可解释终点，才标记为完成。
...
任务必须保存 `next_attempt_at`；不能只在进程内 sleep。
...
至少区分 viewer/operator/admin；站点规则发布、删除、Profile 回滚、手工重抓、Knowledge Sink 变更必须写 audit log。
...
`discovery_run -> fetch_task -> snapshot -> extraction_run -> article_version -> index/publish`
...
Create Site
  ...
  -> Index / Enrichment / Knowledge Sink async
```
...
**FastAPI + Next.js + PostgreSQL + Redis Streams + S3/MinIO + asyncio HTTP Workers + 独立 Playwright/Crawl4AI Browser Worker Pool + DOM/Article IR 抽取层 + 版本化 Markdown Serializer + PostgreSQL/OpenSearch Search Plane。**

对于 1000 个博客站点，最重要的不是选择一个“更强的爬虫库”，而是把站点发现、durable frontier、抓取策略、Browser 资源治理、不可变快照、抽取版本、身份/去重、增量状态机、质量、索引和 Web 运维拆开。这样后续无论增加网站、替换 Crawl4AI、增加新的 Parser、迁移搜索后端或接入外部知识库，都不会重做整条链路，也不会丢失历史抓取和版本证据。
