...
6. 提供 Web 管理端完成站点接入、平台识别、范围确认、规则编辑、任务触发、进度监控、错误诊断、Markdown 预览、版本 diff、元数据质量、搜索质量、安全、预算、Profile/Extraction Contract release 管理。
7. 首次历史回灌和日常增量使用同一套数据模型、任务系统、状态机和可观测体系。
8. HTTP 原始 response bytes、Browser rendered DOM、抽取候选、metadata candidate、规则、Profile、Extraction Contract、质量门禁和 Serializer 全部可追溯。
...
1. **Source Discovery、URL Discovery、Fetch、Capture、Extract、Metadata Resolve、Quality Gate、Asset、Publish、Enrichment 解耦**。
...
6. **不可变抓取快照优先**；规则、Profile、Extraction Contract、抽取器、metadata resolver、质量门禁和 Serializer 变化优先离线 replay。
7. **所有影响结果的 release/version 可追溯**：Platform Profile、Rule、Extraction Contract、Canonicalizer、Fetcher、Capture Profile、Extractor、Metadata Schema、Metadata Resolver、Metadata Analyzer、Quality Gate、Serializer、Security Policy、Chunker、Contextualizer、Embedding、Reranker、Publish Adapter。
...
12. **部分成功显式表达**；正文成功、媒体失败、子 Sitemap 失败、归档未到终点、metadata fallback 失败、索引失败、Rerank 不可用、目标知识库同步失败都不能伪装成完全成功。
...
16. **Profile/Selector/Contract/Metadata 命中率可观测**；模板漂移必须能够一次影响、多站告警。
...
- **抽取面**：Site Selector、多 Extractor Adapter、Metadata Candidate/Resolver、Quality Gate、Article IR、Markdown Serializer。
- **元数据治理面**：字段候选、Schema 校验、字段来源一致性、漂移检测、确定性质量指标、可选 LLM 诊断。
...
| LLM metadata fallback | Crawl4AI `LLMExtractionStrategy` + `LLMConfig` + Pydantic/JSON Schema | 仅 candidate/offline fallback；默认不在每页运行 |
...
| 监控 | Prometheus + Grafana + OpenTelemetry | 系统 SLO + 业务质量 + Metadata/Profile drift + 搜索/发布新鲜度 |
...
- metadata LLM schema fallback 是否允许、触发条件和字段白名单；
...
representative immutable snapshots
 ...
 -> required field/body coverage + metadata agreement + boilerplate + canonical + structure metrics
 -> shadow diff
 -> canary
 -> active release
```
...
### 6.5 Extraction、Metadata 与 Article Version
...
## 11. 多 Extractor、Metadata Resolver、Quality Gate 与 Article IR
...
- 记录 model/provider、prompt/schema version、input/output hash、token/cost、validation error；
- 本地 Ollama 与远程模型只是可替换 Provider Adapter；
- 同一 snapshot 可 `re-run metadata without refetch`；
- 失败只产生 reason code，不阻塞正文发布。

对于稳定页面结构，优先用 Crawl4AI `JsonCssExtractionStrategy` / `JsonXPathExtractionStrategy` 或平台自身 selector；LLM 更适合一次性生成 selector/schema candidate，再经过 golden 验证固化。

### 11.3 Metadata Quality Audit

对 metadata 候选和最终值运行确定性分析：
...
- selector/profile/contract drift；
- LLM candidate validation/fallback/cost。

可选 LLM semantic insight 只生成诊断结论，不修改 canonical metadata。

### 11.4 Platform Quality Gate

综合：正文字数和段落、heading/list/code/table/link/image 保留、boilerplate 比例、链接密度、template fingerprint、重复正文、title/正文一致性、page type、selector/contract 命中、metadata required coverage/conflict、raw/rendered DOM 正文信号、extractor warning 与历史 golden 表现。
...
相同 `snapshot + rule/profile/contract/extractor/metadata resolver/quality/serializer release` 应得到相同 Markdown hash。
...
如果仅 Metadata Resolver/Schema/Analyzer 升级，可基于 immutable snapshot 离线 replay；除非需要新的页面输入，否则不 refetch。
...
Retry：429 尊重 Retry-After；5xx/timeout 指数退避 + jitter；security/robots/access_restricted 不无限重试；parse/extract/metadata 基于 snapshot replay；index/contextualization/embedding/rerank/publish 基于已发布 `article_version` 重试，不重新抓源站。
...
网络 I/O、Browser、CPU 抽取、Metadata LLM fallback、Embedding/Index、Rerank 和 Publish 使用不同资源池。
...
全量 backfill、单站增量、reconciliation、source probing、单 URL 重抓、re-extract without refetch、re-run metadata without refetch、metadata audit、asset retry、export、enrichment、reindex、code reindex、re-contextualize、re-embed、re-publish、cancel/pause/resume。

### 19.4 Rule / Profile / Contract Workbench

支持 draft/testing/publish/rollback、golden snapshot、shadow replay、selector/body/metadata coverage、Markdown diff、canonical/metadata 变化、canary 和 drift。
...
- metadata drift 时间线；
- LLM schema fallback 的 validation/pass/fail/token/cost；
- 一键基于 immutable snapshot 重跑 metadata；
- 从异常样本进入 Contract/Rule Workbench。
...
LLM prompt/response artifact 同样按不可信数据处理，禁止携带 secret；远程模型只接收经过策略允许的内容和字段，默认不把完整 raw HTML 发给外部 Provider。
...
- metadata LLM schema validation/pass/fail/cost；
- metadata drift 和重复率；
...
Redis lag/pending、DB lease/reclaim、PG/S3 latency、HTTP status/latency、Browser slot/RSS/process recycle、Extract CPU/RSS/wall time、domain limiter wait、worker local concurrency、resource pressure state、Metadata LLM latency/token/cost、Embedding API latency/error/cost、Reranker latency/error、Search backend QPS/index throughput、Publish target rate limit/error。
...
- 任意 `article_version` 可解释其抓取、抽取、metadata resolver、索引、发布 provenance；
...
实现 Feed/Sitemap/OrderedArchive、Durable Frontier、conditional HTTP、raw-byte snapshot、Browser fallback、Platform Profile 基础、Article IR、确定性 Metadata Resolver、Markdown、Web CRUD/任务/错误、增量 checkpoint、Egress、预算、日志 sanitizer、Worker 本地资源压力监控与自适应并发。

### Phase 2：历史覆盖、规则与 Metadata 治理

Common Crawl/Wayback、可选 llms.txt、普通归档/Deep Crawl、Discovery Graph / Site Topology、golden/shadow Workbench、Extraction Contract / Schema Synthesis、Profile drift、soft-404、metadata conflict review、Structured Metadata Candidate Adapter、Metadata Quality Console。
...
Create Site
 ...
 -> optional schema-constrained fallback
 -> Metadata Resolver
 -> Quality Gate
 -> Article IR
...
Feed/Sitemap/OrderedArchive/optional llms.txt scan
 ...
 -> extract + metadata resolve + quality gate
...
### 26.3 Metadata Rule/Schema Upgrade

```text
new rule/contract/metadata resolver/schema/model candidate
 -> select immutable golden snapshots
 -> replay without refetch
 -> compare field coverage/conflict/provenance/cost
 -> shadow
 -> canary
 -> activate deterministic release
```

LLM model升级不能直接热替换 canonical resolver；只能作为 candidate generation/fallback generation 先 shadow。
...
- LLM output 必须 JSON parse + schema validate。
- prompt/schema/model/version/input hash 可追溯。
- LLM candidate 不能直接覆盖 canonical metadata。
- remote LLM 默认不接收完整 raw HTML；输入受字段、大小和脱敏策略限制。
...
- Metadata 候选、raw/normalized/final 值和 provenance 可追溯。
- Metadata LLM schema fallback 必须 candidate-only、schema validated、可统计 cost，且可以关闭。
- 支持 `re-run metadata without refetch`。
- Web 可查看 metadata coverage/conflict/drift 并进入 Rule/Contract Workbench。
...
- Profile/Rule/Contract/Extractor/Metadata Resolver 升级可基于历史 snapshot 离线回放。
...
- Rule/Profile/Contract/Extractor/Metadata Resolver/Serializer 变化可解释。
...
- metadata field 来源、冲突和 resolver 决策可复现。
...
```text
A. Metadata Quality
article_version + metadata provenance
...
- Metadata Audit 独立于 canonical 发布，适合 1000+ 站点长期发现模板漂移；
...
这样未来增加站点、替换 Crawl4AI、替换抽取器、替换 LLM provider、切换向量数据库、增加代码检索模型、增加 Reranker 或外部知识库，都不需要重写整个系统，也不会让下游能力反向污染 canonical 知识资产。
