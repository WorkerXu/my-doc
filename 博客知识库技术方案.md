...
8. 最终知识对象保存标题、作者、发布时间、更新时间、标签、canonical、正文块、代码块、表格、图片、附件、链接关系、Snapshot、字段 provenance 与完整 lineage。
9. 支持 durable frontier、断点续跑、幂等、lease、heartbeat、重试、限流、熔断、公平调度、backpressure、dead-letter。
10. 保存不可变网络 Snapshot、渲染 DOM、清洗 HTML、Extraction Candidate、转换轨迹、Canonical IR、Markdown Projection、规则版本与质量结果；规则升级优先离线 replay。
11. 图片和附件走独立 Asset Pipeline，支持安全校验、hash 去重、对象存储、引用关系、独立重试和 Markdown URL 重写。
12. Web 管理端覆盖来源导入、站点接入、Provider 覆盖、任务控制、发现证据、Search Gap、Deep Search Trace、运行时抓取边界、当前文章、历史版本、版本 diff、抽取转换链、质量审核、漂移告警、资产、搜索/RAG、成本、Schedule、AI Workflow 与审计。
...
29. Search Gap Provider 必须保存查询参数、引擎、排名、score、原始结果快照、去重过程与 requested/effective request 参数，不能只保存最终 URL。
30. Search Gap 支持 FAST/DEEP 两级诊断：FAST 仅保存 SERP Evidence；DEEP 只对受预算约束的候选做网页抓取、Chunk 与语义重排，且任何 rerank 结果都不能绕过 Admission 或把 Coverage 提升为 complete。
31. 同一 URL 的多 Extraction Strategy 必须尽可能共享同一 Fetch Snapshot；禁止为了比较 Markdown/HTML/CSS/XPath/LLM 等策略重复访问源站。
32. Search/Rerank 链路保存 `query -> execution -> result -> source fetch -> chunk -> score/rank` 完整证据；Provider Normalizer 不得为了统一字段而丢弃 engine/score/category 等来源元数据。
...
35. Rerank score 必须携带 similarity metric、normalization method、candidate set hash/size、query/document encoding role；候选集内 softmax 一类归一化分数禁止跨不同 candidate set 直接比较。
36. Search/SourceProcessor/Rerank 所有错误路径使用固定 Typed Outcome；禁止成功返回 dict、异常返回 wrapper/空字符串等多态契约。
37. MCP/CLI 只作为调试、人工诊断和 Agent 接口；生产全量/增量同步必须经过 Control Plane、持久任务和 Worker。

## 2. 非目标与边界

1. 不绕过登录、付费墙、验证码、WAF 或明确访问控制。
2. 搜索引擎结果只作补充发现证据。
3. 不承诺未知站点仅靠自动探测达到 100% 历史完整；必须展示 coverage confidence、known gap 与 blocker。
...
11. 不把全局 `reload + scroll + wait_for(main)` 等浏览器脚本作为所有站点默认策略；只能作为 Site/Profile 级可测试 recipe。
...
14. 不把 Query-time Context、Rerank Top-K 或 LLM 摘要反向保存为 Canonical Document。
15. 不把不同候选集合中的 normalized rerank score 当作绝对统一分数。
...
11. `max_depth/max_pages/max_runtime/max_urls/max_results/max_sources/top_k` 是预算，不是 Coverage Evidence。
...
17. 原始证据与面向 Agent/UI 的紧凑输出分层：先保留完整事实，再做字段投影与 token 优化。
...
21. Search 的 requested config、effective config 与 Provider response semantics 分层保存，防止 UI 参数、客户端 clamp 与真实请求混淆。
22. Reranker 的模型名不足以描述运行语义；query/document role、instruction、metric、normalization 与 candidate set 均必须可审计。
...
HTTP、Browser、PDF/OCR、Extraction、PostProcess、Quality、Identity 分为独立 Worker Pool。Browser Worker 通过 Engine Adapter 调用 Crawl4AI/Playwright；Search Gap Worker 通过 Search Adapter 调用 SearXNG/其他 Provider；Deep Search/Rerank Worker 对受预算约束候选执行标准抓取、Chunk 与语义重排。

### 4.4 Projection Plane

从 Accepted Document Version 构建 Markdown、Current Projection、Chunk、Embedding、Search、AI Artifact 与 Delivery；从 Search Evidence 构建 Web/MCP 紧凑结果 Projection；从 Deep Search 结果构建 Rerank Evidence，但不覆盖 Canonical Document。
...
snapshot(body_hash, final_url, fetched_at_bucket)
...
search_rerank_evidence(query_id, search_result_evidence_id, chunk_ref, reranker_release_id, candidate_set_hash)
```
...
站点覆盖状态：`complete | likely_complete | partial | blocked | unknown`。

FULL_BACKFILL Finalizer 必须对 Provider termination evidence、Candidate Manifest、Fetch Manifest、Quality Manifest 对账。Search Gap 结果只增加 Gap Evidence，不能独立把 Coverage 从 partial/unknown 提升为 complete。

## 10. Search Gap 与 Deep Search Diagnostic Plane

### 10.1 定位

Search Provider 主要用于：

- Sitemap/Archive/Feed 数量或日期覆盖异常时发现旧 URL；
- canonical/域名迁移后寻找历史来源；
- 发现仍被搜索引擎索引但官方 Provider 未枚举的页面；
- 人工在 Web 管理端诊断站点缺口；
- 对大量候选做优先级排序。
...
### 10.3 Query Plan 与 Search Execution Manifest
...
每次实际请求另存 `search_execution_manifest`：
...
任何 Agent/MCP/UI 的 `max_sources` 都不能自动等价为 Provider 实际 `num_results`。所有默认值、clamp、固定第一页等必须可见。
...
`provider_fields_json` 保存 engine/engines、score、category、position、provider-specific id 等专有字段；统一接口不得把这些字段永久丢弃。
...
Deep Fetch 必须复用标准 HTTP/Browser Worker、连接池、Site 限流、Scope、SSRF、robots 与安全策略，不允许在 Agent Tool 内另起无状态爬虫旁路生产体系。
...
如果 normalization 为 softmax 或其他候选集内归一化，`normalized_score` 只在相同 `candidate_set_hash + reranker_release_id + normalization_method` 范围内可比较。跨不同 candidate set 排序或阈值判断应使用具有明确语义的 raw score、独立校准分数或重新统一重排。

### 10.7 Reranker Release

Reranker 不能只保存“模型名称”，至少包含：

```text
reranker_kind=EMBEDDING_DOT_PRODUCT|COSINE|CROSS_ENCODER|PROVIDER_NATIVE_RERANK
provider
endpoint_ref
model_name
model_digest
...
必须显式区分 query/document role。若底层 SDK/模型对 query 与 document 使用不同 instruction 或 task，不允许依赖函数默认参数隐式决定。

### 10.8 Typed Outcome 与错误语义

Search、Deep Fetch、Chunk、Rerank 每个阶段统一：
...
禁止错误路径返回与成功路径不同的对象类型，也禁止用 `""`、`[]` 掩盖 schema/contract error。
...
RERANK_STAGE_CONTRACT_ERROR
RERANK_DEADLINE_EXCEEDED
```

429/503 尊重 `Retry-After`，其余瞬时错误指数退避 + jitter。Search/Rerank 故障只降低 Gap Evidence，不影响 Source Sync 主链路。

### 10.9 连接池与运行时

Search/Rerank Worker 按 Provider Release 复用长生命周期 `httpx.AsyncClient`/aiohttp client，配置 connection limit、keep-alive、connect/read/write/pool timeout、Provider 级 semaphore/token bucket 与 circuit breaker。同步 `requests` 或每个 query 新建完整 client 只允许用于低频调试工具，不允许用于生产批量 Reconcile。
...
依靠幂等消重。Feed/Signal 可高频，Sitemap 数小时至每日，Reconcile 每周，深度 Coverage Reconcile 每月或按风险动态调整。

## 12. Fetch Route：HTTP-first 与 Browser 升级

### 12.1 HTTP Worker

使用长期 async client pool，配置 per-host connection、keep-alive、connect/read/write/pool timeout、response bytes limit、redirect limit、conditional request、Retry-After、circuit breaker。
...
禁止每抓一个 URL 创建并销毁完整 Browser/Crawler Runtime。单 URL 调试 API 可以短生命周期，生产批量必须复用 runtime/context，并以 tenant/site 隔离 cookie。
...
只有 representation 证据表明需要 JS 渲染时才新增 Rendered Snapshot。禁止为了执行 `markdown/html/css/xpath/LLM` 等 Strategy 对同一页面重复发起网络请求。
...
`wait_until / js_code / scroll / reload / wait_for / target_elements / excluded_selector` 全部进入 Site/Profile Release。每个 recipe 必须通过 Golden Corpus 与 shadow replay。
...
若未来确需抓取已获授权的登录态内容，Playwright `storage_state` 必须作为加密 Secret Artifact 管理，记录 site/tenant scope、创建时间、过期时间、版本与撤销状态；不得直接进入普通 Profile、日志、Snapshot 或 MCP 输出。Browser Context 必须按租户/站点隔离。
...
timeout/cancel 必须向 HTTP request、Search request、Browser Page/Context、第三方 crawler coroutine、Reranker 调用传播，并持久化 `*_DEADLINE_EXCEEDED` Outcome，释放 lease、连接与页面资源。重试只针对可恢复错误；幂等写入防止重复版本。
...
search-gap/{site_id}/{run_id}/{query_id}/execution.json
...
LLM extraction 只作为版本化 Candidate/Projection，不能成为唯一 Canonical 真相；模型升级不得要求重新访问源站。

内部块使用稳定 block id/hash。Exact duplicate、导航重复、模板重复只形成 Repetition Evidence；默认 Canonical IR 保留原始块。若需要“去重版 Markdown”，作为可重建 Projection 生成，并记录 removed block ids、原因、比例与 Guardrail。
...
- Search/SourceProcessor 是否发生返回类型契约漂移；
- Reranker query/document role 是否与 Release 一致。
...
图片/附件独立下载，校验 content-type、大小、hash、病毒/安全策略；对象存储 hash 去重。Document Version 保存引用关系与原 URL，Markdown Projection 根据部署模式重写为对象存储/CDN URL。Asset 失败单独重试，不阻断正文版本接受，除非 Site Policy 要求附件必需。

## 21. 调度、队列与扩缩容

Schedule Materializer 只把计划物化为持久 Run/Task；Outbox Relay 投递 Redis Streams。Worker 通过 lease/heartbeat 领取任务，支持幂等重试、dead-letter、公平调度与 per-site token bucket。

Browser、HTTP、Search Gap、Search Deep/Rerank、OCR、Embedding 分独立队列。根据 queue lag、active pages、active search/rerank connections、rerank queue latency、CPU/RAM、429 比率、site/provider budget 扩缩容。单站点并发/速率上限优先于全局吞吐。
...
- 展示 requested/effective Search 参数与 client clamp；
...
可查看 Provider 原始 engine/score/category、Reranker Release、Chunk 来源块、candidate set hash/size、normalization、query/document encoding role 与 rank before/after。
...
- Quality 告警与人工 Override；
- 多个 Extractor 对同一 Snapshot 的结果差异。
...
技术博客 Chunker 必须保留代码块边界、标题层级与表格结构；超长代码块可单独形成 code chunk。Chunker 变更只触发 REINDEX，不访问源站。
...
对于 embedding-based rerank，若 query/document 需要不同 instruction/task，Adapter 必须显式调用相应 role；不能依赖 SDK 默认参数。对候选集内 softmax 等分数，UI 必须标明“相对分数”，禁止跨 Run 直接比较。

Embedding/Reranker 变更不触发重新抓源站。
...
search_effective_request_mismatch_total
search_stage_contract_error_total
search_rerank_candidate_count
search_rerank_latency
rerank_error_rate
rerank_candidate_set_change_total
rerank_role_encoding_mismatch_total
...
- 控制面 99.9% 可用；
- accepted task 不丢失；
- Source Sync durable state 可从 PostgreSQL + 对象存储恢复；
- Browser worker 崩溃不影响已提交 Snapshot；
- Search/Rerank Provider 故障不影响主同步链路；
- Search 原始证据、实际请求参数、Deep Fetch、Rerank Evidence 与 Projection 可通过 query_id 完整回溯；
...
2. Contract Test：所有 Stage Representation Contract，尤其验证成功/错误路径返回同一 typed schema。
...
6. Search Execution Manifest Test：验证 requested/effective 参数、默认值、clamp、固定页码等运行事实均可回溯。
7. Provider Provenance Test：Canonical Envelope 生成后仍能回溯 engine/score/category/provider-specific fields 与 Raw Response。
8. Search Deep Test：Top-N 仅作为预算，Deep Fetch 必须走标准 Fetch Route，不得直接发布正文或提升 Coverage。
9. Snapshot Reuse Test：同一 URL 多 Extraction Strategy 默认只产生一次对应 representation 的网络 Snapshot；离线策略比较不能重新抓源站。
10. Reranker Contract Test：保存模型版本、原始分数、归一化分数、rank before/after、Chunk 来源块、candidate set hash/size、metric 与 normalization。
11. Reranker Role Test：固定样本分别验证 query/document encoder mode、instruction 与 task，防止 role 被 SDK 默认参数静默改变。
12. Score Semantics Test：验证 softmax 等 normalized score 不跨 candidate set 比较，并检查 candidate_set_hash。
13. Shadow Replay：新 Profile/Extractor/PostProcessor 对最近 Snapshot 离线回放。
14. Failure Injection：Search/Rerank timeout、bad response、返回类型错误、Browser crash、Worker lease timeout、Redis 重投。
15. Canary：先 1% 站点再逐步放量。
16. Drift Test：正文长度、selector 命中率、重复率、Browser 升级率异常自动回滚/告警。
17. Dependency Capability Test：升级 Crawl4AI/FastMCP/httpx/Playwright/Reranker 后验证 Host/Origin guard、strategy/stream、return shape、timeout、连接池与 score 语义。
18. QA Evaluation 与 ingestion correctness 分离：问答 benchmark 只能评价最终答案，不替代 Coverage、Durability、Provenance 与 Contract 测试。
...
- HTTP/Search/Rerank Client：httpx/aiohttp 长连接池
- Browser：Playwright，Crawl4AI 作为可替换 Engine Adapter
- Search Gap：自托管 SearXNG + 可插拔其他 Search Provider
- Extraction：JSON-LD/CSS/XPath + trafilatura/readability/Crawl4AI candidate，多候选质量选择；LLM extraction 只作可版本化 Candidate
...
HTTP、Browser、OCR、Embedding、Rerank 属于不同资源画像，必须分队列与 Worker Pool。Browser/Rerank 可按 CPU/GPU/内存资源独立扩缩容，不能与轻量 Discovery Worker 混在同一并发池。
...
Redis Streams Worker Pool、per-site 限流、lease/heartbeat、HTTP/Search/Rerank Client Pool、Browser Runtime Pool、Asset Pipeline、Reconcile、版本 diff、OpenSearch/pgvector、监控。

### Phase 3：完整性与诊断

Coverage Evidence、Search Gap FAST/DEEP、Search Provider、迁移/旧 URL 诊断、Provider 差集、Search Gap Web 视图、Deep Search Trace、Search count semantics、Search Execution Manifest、Raw Provider provenance、Rerank Evidence、candidate-set-aware score、role-aware encoding、Runtime Attestation、Crawler Quirk Registry、Capability Test。
...
3. 任意文档可从 Current 追溯到 Document Version、Canonical IR、Extraction Candidate、Snapshot 与 Discovery/Search Evidence。
4. 断电/Worker 崩溃后任务可恢复，不重复发布版本。
5. Incremental 对不变内容不重复跑昂贵 Projection。
6. Search Gap 找到的 URL 不会绕过 Admission/Scope/Fetch/Quality。
7. Browser 不为每个 URL 重启完整 runtime；Search/Rerank Worker 不为每个 query 新建完整 client；1000 站点并发由持久队列、per-site/provider 限流与资源池控制。
...
13. 每次 Search 可同时看到 requested/effective 参数，`max_sources`、Provider `num_results`、客户端 clamp 与实际分页不会被混为一谈。
14. Crawl4AI/FastMCP/httpx/Playwright/Reranker 等依赖升级后必须通过 Capability Test，运行时事实与声明配置不一致时阻止 Release 发布。
15. 同一 URL 的多 Extraction Strategy 默认共享同一 Fetch Snapshot，不得因策略比较重复访问源站。
16. SEARCH_GAP_DEEP 的 Top-N 与 Rerank Top-K 只代表预算和优先级，不得直接决定历史完整性或 Canonical 正文取舍。
17. 任意 Rerank 结果都能追溯到 Query、Search Execution、原始 Search Response、Search Result Evidence、Source Snapshot、Chunk、Reranker Release、原始/归一化分数与 candidate set。
18. normalized rerank score 若属于候选集内归一化，则不会跨不同 candidate set 被直接比较或作为全局阈值。
19. Reranker query/document role、instruction、metric 与 normalization 均可通过 Release + Runtime Attestation 复现。
20. Provider Normalizer 不会因统一输出而永久丢失 engine、score、category 等专有字段。
21. Deep Search Fetch 必须走标准 Scope、SSRF、robots、限流、Retry、Snapshot 与 Audit 链路。
22. 离线 REPROCESS/Extractor 对比可以完全基于已有 Snapshot 完成，不访问源站。
23. Search/SourceProcessor/Rerank 在成功、异常和超时路径均遵循固定 Typed Outcome，不会因 broad catch 返回另一种对象形态。
24. Web 管理端可以从站点 Coverage 下钻到单个 Provider、Query、Search Execution、Candidate、Fetch、Chunk、Rerank、Document Version 与 Projection，所有关键决策均可审计。
25. QA benchmark 与 ingestion correctness 独立评估；回答质量提升不能掩盖 Coverage、Provenance、Durability 或 Contract 回归。