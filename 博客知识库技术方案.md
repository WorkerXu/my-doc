...
PostgreSQL 对 active run 建部分唯一索引，防止 cron 重复、Leader 切换、HTTP 重试、Web 双击或 Outbox 重放制造重复执行。禁止只采用“先查再插”的应用层去重。

- `Run Now`：若同 site/mode 已有 active run，返回现有 run。
...
禁止任务先标记成功再异步补写对象。REPROCESS 默认只使用已有 snapshot，不访问网络。
...
同一 PostgreSQL 事务写业务状态和 outbox_event；Relay 再投递 Redis Streams。重复消息必须安全重放；ack 只能发生在持久化成功之后或遵循明确 inbox/outbox 协议。
...
`sync_schedule` 管未来触发，`crawl_run` 管一次逻辑执行，`background_task` 管可领取工作单元，三者不得合并成一张表。
...
统一分类 DNS、connect/read timeout、TLS、401/403、404/410、429、5xx、robots denied、egress denied、content too large、sitemap malformed、depth anomaly、decompression bomb、browser crash/timeout、extraction failed、quality rejected、runtime mismatch、artifact commit failed、schema mismatch、lease lost、scheduler duplicate trigger、secret resolution failed 等。
...
Web 能定位任一 schedule 的下次触发、最近 run、dedupe key、supersede/cancel 关系，以及任一任务的 worker、lease、heartbeat、attempt、恢复事件和完整 event timeline。
...
- Scheduler 到期只创建 run + outbox，不直接抓取。
...
该方案可从几十个站点平滑扩展到 1000+ 站点，并在持续增加新站点、新发现方式、新抓取引擎和新同步频率时保持核心状态机稳定，同时保证长期增量同步在 Scheduler 重启、Worker 崩溃、消息重放和多副本部署下仍可恢复、可去重、可审计。