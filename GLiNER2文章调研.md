...
| Superlinked SIE 的电商搜索示例把 GLiNER 系列抽取器直接放进在线检索链路：先从商品描述抽结构化 attributes.brand，再做向量编码、检索和 rerank，并支持品牌过滤；适合参考 GLiNER2 的“商品文本→品牌/属性→结构化元数据→搜索过滤”服务接口与工程拆分。 | https://github.com/superlinked/sie/blob/main/examples/ecommerce-product-search/typescript/README.md |
| OpenWeb 的 Costco 适配器从真实商城 JSON 中提取商品 title、brand、category 和 attributes，并明确用 attributes.Brand 规避脏 manufacturer 字段；适合给 GLiNER2 品牌/型号/规格抽取增加“结构化来源优先、语义模型补抽”的字段优先级与真值校验策略。 | https://github.com/imoonkey/openweb/blob/main/src/sites/costco/DOC.md |
...
| Etsy 的生产实践把商品标题、描述、图片、类目 taxonomy 与类目专属规则一起作为上下文，将亿级 Listing 的长尾商品信息抽成结构化属性，并通过银标评测、Pydantic/业务规则校验与大规模并发推理提升覆盖；很适合参考 GLiNER2 的“类目 Schema→多字段抽取→字段校验→难例评测”生产闭环。 | https://www.etsy.com/codeascraft/understanding-etsyas-vast-inventory-with-llms |
| myIR 的 2026 工程实践把电商页面从通用文档检索升级为 ProductDetail/ProductCard 结构化抽取，直接输出 brand、SKU、description、availability 等字段，并对缺失 SKU 等异常返回 warnings；适合放在 GLiNER2 前后作为网页定位、字段契约与质量告警层，让品牌/型号/属性结果进入稳定 PDP 解析流程。 | https://medium.com/@jsanca/from-information-retrieval-to-product-extraction-reusing-myir-to-understand-an-e-commerce-site-79922c6c9d10 |
| 该 2026 Magento 目录富化实践把脏商品数据映射到 EAV 属性体系，使用 AI 做属性抽取、值标准化和类目属性集选择，并将结果批量回写；适合参考 GLiNER2 的“动态 Schema/属性集→字段抽取→canonical value→PIM 入库”链路，尤其处理 Blk/Black、med/Medium 等同义值归一。 | https://medium.com/@hayrikar54/the-0-pim-automating-magento-catalog-enrichment-with-ai-and-google-sheets-ce067114e18f |
| 该 eBay ML Challenge 项目用 spaCy NER 从 Listing 标题/描述直接抽 Brand、Model、Color、Condition、Size，并报告 weighted Fβ 0.83；字段与 GLiNER2 商品 Schema 高度同构，适合作为品牌/型号/属性 span 抽取的监督基线和回归集。 | https://github.com/ysingh1305/eBayMLChallenge |
| SpiceWorx RFQ Assistant 把工业品自然语言询盘抽成 Category、Brand、Model、Specifications、Quantity 等 JSON，并返回置信度、缺失字段和追问；适合参考 GLiNER2 在 B2B 商品文本中的动态 Schema、缺失值门禁和低置信补问。 | https://www.spiceworx.com/en/rfq-assistant-explainer-technical.html |
...
| 该 ACL 工作在 3.7 万条人工标注购物查询上直接识别 product、brand、model、product family，并给出这些相近实体的业务边界；非常适合用来校准 GLiNER2 的品牌/系列/型号 Schema，尤其测试短查询中 Samsung Galaxy 这类“品牌+产品族+型号”的边界切分。 | https://aclanthology.org/P16-2018/ |
| 该土耳其语电商 NER 数据集提供 1,700 条商品名，字符级实体直接包含 BRAND、MODEL、CATEGORY、COLOR、SIZE_VARIANT、MATERIAL、SPECIFICATION 等；可原样映射为 GLiNER2 动态 Schema 的多语言回归/少样本集，专项验证字母数字型号、容量和规格 span。 | https://huggingface.co/datasets/gururaser/turkish-ecommerce-ner-dataset |
