...
```mermaid
flowchart TD
    S[计算候选对最终 score] --> C1{"score >= T_auto?"}
    C1 -- 是 --> A[auto_accepted：自动合并并进入实体组]
    C1 -- 否 --> C2{"score >= T_review?"}
    C2 -- 是 --> R[review：进入人工复核]
    C2 -- 否 --> X[rejected：不合并]
```

**图 1：阈值判定阶梯——根据最终分数落入自动合并、人工复核或不合并三档。**
...
**图 2：系统四层架构与数据流，人工复核结果以黄金标签回流到匹配与阈值调优。**
...
**图 3：一次批处理的跨组件时序，审核结果以黄金标签回流到匹配判定。**
...
**图 4：传递一致性清洗算法流程——对长度 2 路径做隐含比较，删除低分边或标记冲突并迭代重算。**
...
**图 5：判定与审核状态机——auto_accepted、review、rejected 三态及人工审核操作带来的状态流转；conflict_flag 作为实体组属性而非图中状态。**
...
**图 6：核心数据表实体关系图——明确候选对、实体组、成员、被删边与审核事件之间的外键与基数。**
