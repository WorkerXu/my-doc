...
判定“同一商品”时，官方属性按以下规则比对：

| 判定对象 | 同一商品必须一致 | 不允许合并的差异 |
| --- | --- | --- |
| 品牌 | 一致 | 品牌不同 |
| 系列 / 子系列 | 一致 | 同系列不同型号 |
| 型号 / Reference | 一致 | 不同型号 |
| 关键区分属性（腕表） | 表径、表壳材质、表盘颜色、机芯一致 | 不同尺寸 / 颜色 / 材质 / 盘面 |
| 关键区分属性（箱包） | 官方尺寸、材质、五金、版本一致 | 普通皮与鳄鱼皮、金 / 银五金不同 |
| 官方编码 | — | 官方 SKU / variant 不同 |
...
系统用 `BAAI/bge-m3` 编码文本。bge-m3 输出稠密向量与稀疏向量。稠密向量衡量语义相似。稀疏向量命中型号、品牌等精确词。

“抽取后结构化文本”的编码做法：

1. 按固定顺序拼接 GLiNER2 抽出的字段。顺序为：品牌、型号、系列、材质、尺寸、颜色、机芯。
2. 记录缺少某个字段时，该字段留空，不写入。
3. 用同一个 `BAAI/bge-m3` 编码拼接后的文本。
4. 标题与结构化文本都编码成向量，共用同一模型，保证向量可比。
5. 相似度等于稠密向量余弦相似度与稀疏向量命中的结果。
...
系统导出 JSON 供程序消费。实体组结果存入结构化表，供审核与查询。
...
| 表名 | 用途 | 关键字段 |
| --- | --- | --- |
| `raw_records` | 来源原始记录与字段快照 | `source_id` · `product_id` · `title` · `raw` |
| `extracted_attributes` | GLiNER2 抽取结果与置信度 | 实体类型 · 实体文本 · 置信度 |
| `candidate_pairs` | 候选对与相似度证据 | 候选对 · 相似度 · 命中字段 |
| `entity_groups` | 实体组与 canonical 记录 | `entity_id` · canonical |
| `entity_members` | 实体组与来源成员映射 | `entity_id` · 来源记录 ID |
...
- 语义向量模型用 `BAAI/bge-m3`。用 int8 量化，可在纯 CPU 上运行。
...
| 阶段 | 主要活动 | 交付物 |
| --- | --- | --- |
| 抽样验证 | 三来源抽样、标注黄金标签、跑通 GLiNER2 抽取与实体组输出 | 实体组级别的精确率 / 召回率报告 |
| 全量批处理 | 全量入 Iceberg，分布式跑抽取、候选生成、匹配、聚类 | 实体组结果集，进入人工抽检 |
| 增量与闭环 | 日常增量更新，审核结果回流为标注数据，必要时用 adapter 或轻量分类器优化 | 持续更新的统一商品库 |
