...
| --- | --- | --- | --- |
| 品牌 + 分类 + 规范化型号都存在且相同 | `exact_model` | 是（进入匹配判定） | 只缩小候选范围，不代表同一商品 |
| 型号缺失或置信度低，品牌 + 分类存在 | `semantic_top_k` | 是（进入匹配判定） | bge-m3 + FAISS Top-K |
| 仅品牌缺失 | 可生成候选 | 否 | 仅进入人工复核 |
| 仅分类缺失 | 品牌 + 规范化型号 | 进入匹配判定 | 原文未单独限制 |
...
如果型号缺失或型号置信度低，系统在品牌和分类桶内使用 `BAAI/bge-m3` 向量和 FAISS 进行 Top-K 语义召回。`K` 是配置项，默认 5 至 10，抽样阶段调优。
...
**图 4：判定与审核状态机——Selecting 输出决定初始状态，人工审核操作带来后续流转；conflict_flag 作为实体组属性而非图中状态。**
