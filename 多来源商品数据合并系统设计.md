...
匹配打分只使用语义相似度，不做字段一致性比较。

语义相似度用两种文本计算：

- 标题原文
- 结构化文本：按固定顺序拼接 GLiNER2 字段。顺序为品牌、型号、系列、材质、尺寸、颜色、机芯。记录缺失字段时留空，不写入。

两种文本都用 `BAAI/bge-m3` 编码。bge-m3 输出稠密向量与稀疏向量。稠密向量衡量语义相似。稀疏向量命中型号、品牌等精确词。

第一版用相似度阈值判定。第二版用黄金标签训练轻量分类器（梯度提升树或逻辑回归）。每个候选对必须保留相似度明细，供人工审核。
